...
void test_is_sorted_until(Policy&& pol, std::size_t problem_size,
  ...
  auto ret = std::is_sorted_until(pol, data.begin(), data.end());
  auto ret_host = std::is_sorted_until(data.begin(), data.end());

  BOOST_CHECK(ret == ret_host);

  ret = std::is_sorted_until(pol, data.begin(), data.end(), comp);
  ret_host = std::is_sorted_until(data.begin(), data.end(), comp);
...
void medium_size_tests(Policy&& pol) {
  test_is_sorted_until(pol, 200, [](int i){return -i;});
  test_is_sorted_until(pol, 200, [](int i){return -200 + i;});
  test_is_sorted_until(pol, 200, [](int i){return (i > 50 ? i : 200-i);});
  test_is_sorted_until(pol, 200, [](int i){return (i <= 50 ? i : 200-i);});
  test_is_sorted_until(pol, 200, [](int i){return (i == 0 ? i : 200-i);});
  test_is_sorted_until(pol, 200, [](int i){return i;});
  test_is_sorted_until(pol, 200, [](int i){return 42;});
}
