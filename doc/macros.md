...
* `HIPSYCL_EXT_AUTO_PLACEHOLDER_REQUIRE` - define before including `sycl.hpp` to activate the extension for automatically calling `require()` on specified placeholder accessors in submitted command groups. See the [extension documentation](extensions.md) for more details.
