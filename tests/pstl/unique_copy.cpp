...
BOOST_FIXTURE_TEST_SUITE(pstl_unique_copy, enable_unified_shared_memory)

template<class T, class Policy, class Generator, class BinaryPredicate = std::equal_to<>>
void test_unique_copy(Policy&& pol, Generator&& gen, std::size_t problem_size,
  ...
  auto ret = std::unique_copy(pol, data.begin(), data.end(), dest_device.begin(), pred);
  auto ret_reference = std::unique_copy(data.begin(), data.end(), dest_host.begin(), pred);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(par_unseq_empty, T, types) {
  test_unique_copy<T>(std::execution::par_unseq, [](int i){return i+1;}, 0);
}

BOOST_AUTO_TEST_CASE_TEMPLATE(par_unseq_single_element_match, T, types) {
  test_unique_copy<T>(std::execution::par_unseq, [](int i){return 42;}, 1);
}

BOOST_AUTO_TEST_CASE_TEMPLATE(par_unseq_single_element, T, types) {
  test_unique_copy<T>(std::execution::par_unseq, [](int i){return i+1;}, 1);
}

BOOST_AUTO_TEST_CASE_TEMPLATE(par_unseq_medium_size_none, T, types) {
  test_unique_copy<T>(std::execution::par_unseq, [](int i){return i+1;}, 20);
}

BOOST_AUTO_TEST_CASE_TEMPLATE(par_unseq_medium_size_all, T, types) {
  test_unique_copy<T>(std::execution::par_unseq, [](int i){return 42;}, 20);
}

BOOST_AUTO_TEST_CASE_TEMPLATE(par_unseq_medium_size_half, T, types) {
  test_unique_copy<T>(std::execution::par_unseq,
                      [](int i){return (i%5 != 0 ? 42: i);}, 20);
}

BOOST_AUTO_TEST_CASE_TEMPLATE(par_empty, T, types) {
  test_unique_copy<T>(std::execution::par, [](int i){return i+1;}, 0);
}

BOOST_AUTO_TEST_CASE_TEMPLATE(par_single_element_match, T, types) {
  test_unique_copy<T>(std::execution::par, [](int i){return 42;}, 1);
}

BOOST_AUTO_TEST_CASE_TEMPLATE(par_single_element, T, types) {
  test_unique_copy<T>(std::execution::par, [](int i){return i+1;}, 1);
}

BOOST_AUTO_TEST_CASE_TEMPLATE(par_medium_size_none, T, types) {
  test_unique_copy<T>(std::execution::par, [](int i){return i+1;}, 20);
}

BOOST_AUTO_TEST_CASE_TEMPLATE(par_medium_size_all, T, types) {
  test_unique_copy<T>(std::execution::par, [](int i){return 42;}, 20);
}

BOOST_AUTO_TEST_CASE_TEMPLATE(par_medium_size_half, T, types) {
  test_unique_copy<T>(std::execution::par,
                      [](int i){return (i%5 != 0 ? 42: i);}, 20);
}
