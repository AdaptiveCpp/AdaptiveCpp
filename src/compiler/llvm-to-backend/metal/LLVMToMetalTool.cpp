...
createMetalTranslator(const hipsycl::common::hcf_container& HCF) {
  std::vector<std::string> KernelNames;
  if(!tool::getHcfKernelNames(HCF, KernelNames)) {
    return nullptr;
  }
  return hipsycl::compiler::createLLVMToMetalTranslator(KernelNames);
}

int main(int argc, char* argv[]) {
  return tool::LLVMToBackendToolMain(argc, argv, createMetalTranslator);
}
