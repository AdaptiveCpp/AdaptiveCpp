...
void InclusionRewriter::Process(FileID FileId,
  ...
  hipsycl::transform::RewriteSelector fileSelector;
    ...
    if (RawToken.is(tok::hash) && RawToken.isAtStartOfLine()) {
      ...
      if (RawToken.getIdentifierInfo() != nullptr) {
        switch (RawToken.getIdentifierInfo()->getPPKeywordID()) {
          ...
          case tok::pp_import: {
            // Changes introduced for hipSYCL to only rewrite user includes
            ...
            if (Mod)
              ...
              const Module* IncMod = FindEnteredModule(Loc);
              if (IncMod)
                OS << "#pragma clang module begin "
                   << IncMod->getFullModuleName(true) << "\n";
...
void hipsycl::transform::RewriteUserIncludesInInput(Preprocessor &PP, raw_ostream *OS,
