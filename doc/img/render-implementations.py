...
def add_backend(graph,
                id_name,
                color,
                supported_hardware_colors,
                description=None,
                device_caveats={}):
    ...
    graph.node(id_name,
               ...
               fontcolor=str(determine_font_color(color)))
    ...
    if display_mode == "no-hardware-nodes":
        ...
        for hw in supported_hardware_colors:
            ...
            graph.node(hw_id,
                       hw,
                       shape='box',
                       color=str(supported_hardware_colors[hw]),
                       style='rounded,filled',
                       width=default_node_width,
                       fontcolor=str(
                           determine_font_color(
                               supported_hardware_colors[hw])))
            if hw in device_caveats:
                graph.edge(id_name,
                           hw_id,
                           penwidth='2',
                ...
                graph.edge(id_name,
                           hw_id,
                           penwidth='2',
                           tailport=default_tailport,
                           headport=default_headport,
                           color=str(color))


def add_implementation(graph,
                       id_name,
                       color,
                       secondary_color,
                       backends,
    ...
    for b in backends:
        if b in backend_caveats:
            graph.edge(id_name,
                       ...
                       style="dashed",
                       headlabel=add_footnote(str(backend_caveats[b])),
                       labelfloat="true",
                       labeldistance="2.0",
                       weight="100")
        else:
...
if __name__ == '__main__':
    ...
    graph.node("root",
               "SYCL source code",
               shape='box',
               color=str(root_color),
               style='rounded,filled',
               fontcolor=str(determine_font_color(root_color)))
    ...
    add_backend(graph,
                "dpcpp_cuda",
                intel_blue, {'NVIDIA GPUs': nvidia_green},
                description="CUDA\n(driver API)")

    add_backend(graph,
                "dpcpp_l0",
                intel_blue, {'Intel GPUs': intel_blue},
                description="Level Zero")

    add_implementation(
        graph, "DPC++", intel_blue, intel_blue,
    ...
    add_backend(graph,
                "hipsycl_cuda",
                hipsycl_grey, {'NVIDIA GPUs': nvidia_green},
                description="CUDA\n(runtime API)")

    add_backend(graph,
                "hipsycl_rocm",
                hipsycl_grey, {'AMD GPUs': amd_red},
                description="ROCm")

    add_backend(graph,
                "hipsycl_openmp",
                hipsycl_grey, {'Any CPU': cpu_color},
                description="OpenMP")

    add_backend(graph,
                "hipsycl_l0",
                hipsycl_grey, {'Intel GPUs': intel_blue},
                description="Level Zero")
    ...
    add_backend(graph,
                "neosycl_veo",
                nec_blue, {'NEC SX-Aurora TSUBASA': nec_blue},
                description="VEO")
    ...
    add_backend(graph,
                "computecpp_host",
                codeplay_purple, {'Any CPU': cpu_color},
                description="Host")

    add_backend(graph,
                "computecpp_ptx",
                codeplay_purple, {'NVIDIA GPUs': nvidia_green},
    ...
    add_backend(graph,
                "sycl_gtx_opencl",
                sycl_gtx_dark, {'Any OpenCL 1.2 device': sycl_gtx_dark},
                description="OpenCL 1.2")
    add_implementation(graph,
                       "sycl-gtx",
                       sycl_gtx_dark,
                       sycl_gtx_light, ["sycl_gtx_opencl"],
                       caveat='Non-standard macros required')

    graph.node("footnotes",
               footnote_text,
               shape="plain",
               style='filled',
               color="#ffffff00")
