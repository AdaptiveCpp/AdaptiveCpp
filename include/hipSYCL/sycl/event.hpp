...
public:
  ...
  // Wait for and retrieve profiling timestamps. Supports all handler operations
  // except handler::require() and handler::update_host(). Will throw
  // sycl::invalid_object_error unless the queue was constructed with
  ...
  template <typename Param> typename Param::return_type get_profiling_info() const {
    ...
    if (std::is_same_v<Param, info::event_profiling::command_submit>) {
      auto submission = _node->get_operation()
      ...
      auto start = _node->get_operation()
                       ->get_instrumentations()
