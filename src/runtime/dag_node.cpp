...
dag_node::dag_node(const execution_hints &hints, const node_list_t &requirements,
                   std::unique_ptr<operation> op, runtime *rt)
    : _hints{hints}, _assigned_executor{nullptr}, _event{nullptr}, _operation{std::move(op)},
      _is_submitted{false}, _is_complete{false}, _is_virtual{false}, _is_cancelled{false}, _rt{rt} {
...
dag_node::~dag_node() { TRACER_FUNCTION_VA_ARGS(dag_node_destructor, std::hash<void *>{}(this)); }
...
bool dag_node::is_known_complete() const { return _is_complete; }
...
void dag_node::assign_to_executor(backend_executor *ctx) { this->_assigned_executor = ctx; }

void dag_node::assign_to_device(device_id dev) { this->_assigned_device = dev; }

void dag_node::assign_to_execution_lane(void *lane_id) { this->_assigned_execution_lane = lane_id; }
...
backend_executor *dag_node::get_assigned_executor() const { return _assigned_executor; }

void *dag_node::get_assigned_execution_lane() const { return _assigned_execution_lane; }
...
template <class F> void descend_requirement_tree(F &&f, const dag_node *n) {
  if (f(n)) {
...
bool recursive_find(const dag_node_ptr &current, int current_level, const dag_node_ptr &x) {
  if (!current)
    return false;
  if (current == x)
    return true;
...
void dag_node::add_requirement(dag_node_ptr requirement) {
  ...
  auto is_reachable_from = [](dag_node_ptr from, dag_node_ptr to, int max_levels) -> bool {
    return recursive_find(from, max_levels, to);
  };

  const int search_depth = application::get_settings().get<setting::dag_req_optimization_depth>();
  ...
  _requirements.erase(
      std::remove_if(_requirements.begin(), _requirements.end(),
                     [](std::weak_ptr<dag_node> weak_req) { return weak_req.expired(); }),
      _requirements.end());
...
const weak_node_list_t &dag_node::get_requirements() const { return _requirements; }
...
void dag_node::wait() const {
  ...
  descend_requirement_tree(
      [](const dag_node *current) -> bool {
...
std::shared_ptr<dag_node_event> dag_node::get_event() const { return _event; }

void dag_node::for_each_nonvirtual_requirement(std::function<void(dag_node_ptr)> handler) const {
...
runtime *dag_node::get_runtime() const { return _rt; }
